...
class House(BaseModel, db.Model):
    ...
    facilities = db.relationship("Facility", secondary=house_facility)  # 房屋的设施  secondary：处理多对多关系（house_facility中间表）
...
class Order(BaseModel, db.Model):
    ...
    status = db.Column(  # 订单的状态
        ...
        default="WAIT_ACCEPT", index=True)  # index=True：指明给这个字段建立索引
    comment = db.Column(db.Text)  # 订单的评论信息或者拒单原因
    trade_no = db.Column(db.String(80))  # 交易的流水号 支付宝的

    # 订单状态
    order_status_li = ["CANCELED", "REJECTED"]
